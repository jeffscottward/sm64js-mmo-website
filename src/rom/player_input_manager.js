...
export const sendChatMsg = (msg) => {
  if (msg[0] === "!") {
    ...
    // hacky way to combine chat and ban box
...
window.addEventListener(
  ...
  (e) => {
    ...
    if ($("#playerNameRow").hidden && e.keyCode === 13) {
      $("#signinSection").effect(
        "shake",
        { direction: "down", times: 3, distance: 3 },
        500
      );
    }

    if (
      !window.playerNameAccepted &&
      e.keyCode === 13 &&
      !$("#playerNameRow").hidden
    ) {
...
if (localStorage["controls"]) {
  Object.assign(
    ...
    JSON.parse(localStorage["controls"]).keyboard
  );
  Object.assign(
    gamepadButtonMapping,
    JSON.parse(localStorage["controls"]).gamepad
  );
}
...
window.addEventListener("gamepadconnected", function (e) {
  ...
  $("#noGamepadMessage").hidden = true;
  $("#gamepadMessageDiv").hidden = false;
  document.getElementById(
    "gamepadMessage"
  ).innerHTML = `Detected Gamepad: "${gamepad.id.slice(
    0,
    30
  )}" with ${numButtons} Buttons`;

  ///Fillout the select options and set default value
  const controlsWindowDiv = $("#gamepadControlsWindow");
...
export const playerInputUpdate = () => {
  ...
  if (gamepad) {
    ...
    Object.assign(gamepadFinal, {
      ...
      cr:
        gamepad.axes[gamepadButtonMapping["cStickX"]] &&
        gamepad.axes[gamepadButtonMapping["cStickX"]] > 0.5,
      cl:
        gamepad.axes[gamepadButtonMapping["cStickX"]] &&
        gamepad.axes[gamepadButtonMapping["cStickX"]] < -0.5,
      cu:
        gamepad.axes[gamepadButtonMapping["cStickY"]] &&
        gamepad.axes[gamepadButtonMapping["cStickY"]] < -0.5,
      cd:
        gamepad.axes[gamepadButtonMapping["cStickY"]] &&
        gamepad.axes[gamepadButtonMapping["cStickY"]] > 0.5,
    });
  ...
  window.sm64js.playerInput = {
    ...
    buttonPressedStart:
      buttonDownStart && !window.sm64js.playerInput.buttonDownStart,
    ...
    buttonPressedCl:
      buttonDownCl &&
      !window.sm64js.playerInput.buttonDownCl &&
      !buttonDownTaunt,
    buttonPressedCr:
      buttonDownCr &&
      !window.sm64js.playerInput.buttonDownCr &&
      !buttonDownTaunt,
    buttonPressedCu:
      buttonDownCu &&
      !window.sm64js.playerInput.buttonDownCu &&
      !buttonDownTaunt,
    buttonPressedCd:
      buttonDownCd &&
      !window.sm64js.playerInput.buttonDownCd &&
      !buttonDownTaunt,
    ...
    buttonPressedChat:
      buttonDownChat && !window.sm64js.playerInput.buttonDownChat,
    buttonPressedFreezeCam:
      buttonDownFreezeCam && !window.sm64js.playerInput.buttonDownFreezeCam,
    ...
    taunt: Object.values(tauntCommands).includes(window.taunt)
      ? window.taunt
      : undefined,
  };
  ...
  if (gameData.marioState)
    gameData.marioState.controller = window.sm64js.playerInput;

  if (
    window.sm64js.playerInput.buttonPressedStart &&
    keyboardButtonMapping.start != "enter"
  ) {
    submitPlayerName();
  }
  ...
  if (window.sm64js.playerInput.buttonPressedFreezeCam) {
    window.freezeCamera();
  }
};
